...
var tabs=['d','r','p','e','b','t','s','m'];
tabs.forEach(function(id){el('tb-'+id).addEventListener('click',function(){tabs.forEach(function(t){el('pg-'+t).classList.remove('on');el('tb-'+t).classList.remove('on');});el('pg-'+id).classList.add('on');el('tb-'+id).classList.add('on');if(id==='m')renderMyPage();if(id==='p'){renderPlanner();setupPlannerListener();}if(id==='d')renderDashboard();if(id==='b')renderBills();});});
function switchTab(id){tabs.forEach(function(t){el('pg-'+t).classList.remove('on');el('tb-'+t).classList.remove('on');});el('pg-'+id).classList.add('on');el('tb-'+id).classList.add('on');if(id==='p'){renderPlanner();setupPlannerListener();}if(id==='d')renderDashboard();}
...
function loadPersonal(){if(!userName)return;db.ref('personal/'+userName).on('value',function(snap){personalData=snap.val()||{};if(el('pg-m').classList.contains('on'))renderMyPage();if(el('pg-d').classList.contains('on'))renderDashboard();if(el('pg-p').classList.contains('on'))renderPlanner();});}
